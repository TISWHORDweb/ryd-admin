...
const ManagePartnerProgram = () => {
    ...
    const fetchPrograms = async () => {
        try {
            ...
            const response = await axios.get(`${baseUrl}/admin/partner/program/all`);
                                    ...
                                    <div className="d-flex flex-wrap align-items-center justify-content-end gap-2 mb-3">
                                        ...
                                        <div>
                                            <select className={'form-control'} onChange={(e) => {
                                                ...
                                                {organizations && organizations.map((o, i) => <option key={i}
                                                value={o}>{o}</option>)}
                                            </select>
                                        </div>
                                        <div>
                                        ...
                                        <table className="table align-middle table-hover">
                                            ...
                                            {displayProgramList.map((program, index) => (
                                                <tr key={index}
                                                            ...
                                                            <input style={{marginRight: 5}} type={'checkbox'}
                                                                   onChange={(d) => {
                                                                       ...
                                                                       if (multiIDs.includes(program.id)) {
                                                                           ...
                                                                           toast.error(program?.partner?.organizationName + " have been removed from the action list.")
                                                                       } else {
                                                                           setMultiIDs([...multiIDs, program.id])
                                                                           toast.warn(program?.partner?.organizationName + " has been added to action list.")
                                                                       }
                                                                   }}/>
                                                            {index + 1}
                                                        </div>
                                                    </td>
                                                    <td>{program?.partner?.organizationName}</td>
                                                    ...
                                                    <td>{program?.partner_teacher?.firstName}</td>
                                                    <td>{program?.partner_package?.title}<br/><small
                                                        style={{color: 'red'}}>[{program?.partner_cohort?.title || "No Cohort"}]</small>
                                                    </td>
                                                    {/*<td>*/}
                                                    {/*  {program?.package?.status ? "Active" : "Inactive"}*/}
                                                    {/*</td>*/}
                                                    <td>{program?.partner_package?.level}</td>
                                                    ...
                                                    <td>
                                                        <div style={{width: 50}}>
                                                            ...
                                                            <br/>
                                                            <small style={{fontSize: 10, backgroundColor: "#e3e3e3", padding:2, display: program?.partner_coupon?.code?"block":"none"}}>{program?.partner_coupon?.code}</small>
